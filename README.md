...
이전에 작성했던 JCFService,FileService는 하나의 클래스 안에 비즈니스 로직과 저장 로직이 같이 들어있었는데 저장로직만 따로 분리하여 작성
왜냐하면 저장 방식을 새로 추가하거나 변경하려고 하면 비즈니스 로직까지 건들여야해서
